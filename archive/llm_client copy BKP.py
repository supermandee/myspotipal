...
from spotify_client import get_top_items, get_followed_artists, get_user_playlists, get_saved_shows, get_recently_played_tracks, search_artist, get_artist_info, search_item
import uuid
import requests
import json
...
class LLMClient:
    ...
    def classify_query(self, query):
        messages = [
            ...
            {"role": "assistant", "content": "saved_shows"},
            {"role": "user", "content": "Recommend me some songs based on my listening history."},  # Example recommendation query
            {"role": "assistant", "content": "recommend"},
            {"role": "user", "content": "Recommend me some songs by Taylor Swift."},  # Example recommendation query
        ...
        try:
            ...
            # Fallback to fetch_general_info if classification is "unknown"
            if classification == "unknown":
                print("Query classified as unknown. Falling back to fetch_general_info.")
                return self.fetch_general_info(None, None, query)

            return classification
        
        except OpenAIError as e:
            print(f"Error classifying query with LLM: {e}")
            # Fallback to fetch_general_info in case of error
            return self.fetch_general_info(None, None, query)
        except Exception as e:
            print(f"Unexpected error in classify_query: {e}")
            # Fallback to fetch_general_info in case of unexpected error
            return self.fetch_general_info(None, None, query)
        
    def decompose_query(self, query):
        # Use LLM to identify and split query into components
        messages = [
            {"role": "system", "content": "Decompose this complex query into multiple sub-queries."},
            {"role": "user", "content": query}
        ]
        response = self.client.chat.completions.create(
    ...
    def process_query(self, query, spotify_data, access_token, session_id):
        # Initialize memory for the session if not present
        if session_id not in self.memory:
            self.memory[session_id] = {"history": [], "extracted_entities": {}}
        
        # Add the user's query to memory history
        self.memory[session_id]["history"].append({"query": query, "response": None})

        # Analyze query context to determine if it's a follow-up question
        try:
            context = self.analyze_query_context(query, session_id)
            logging.debug(f"Query context: {context}")
        except Exception as e:
            logging.exception("Error analyzing query context.")
            context = {"is_follow_up": False}  # Fallback if context analysis fails

        # Construct a prompt with context from memory
        try:
            prompt = self.construct_prompt_with_memory(query, session_id)
            logging.debug(f"Constructed prompt: {prompt}")
        except Exception as e:
            logging.exception("Error constructing prompt with memory.")
            prompt = query  # Fallback to the raw query

        try:
            # If this is a follow-up question, handle it with additional context
            if context.get("is_follow_up", False):
                # Retrieve the last relevant response from history
                last_response = next(
                    (msg.get("response") for msg in reversed(self.memory[session_id]["history"]) if msg.get("response")),
                    None
                )

                if last_response:
                    # Create a detailed prompt using the previous context and the new query
                    detailed_prompt = self.create_context_aware_prompt(
                        query=query,
                        context=context,
                    ...
                    logging.debug(f"Detailed prompt for follow-up query: {detailed_prompt}")

                    # Prepare messages for OpenAI API
                    messages = [
                        {"role": "system", "content": "You are a personal Spotify assistant. Use both previous context and new information to provide accurate answers."},
                        {"role": "user", "content": detailed_prompt}
                    ]

                    # Stream and save response
                    return self._stream_and_save_response(messages, session_id, query)

            # If not a follow-up, classify the query and process it as a new query
...
"""
    ...
    def construct_prompt_with_memory(self, query, session_id):
        history = self.memory.get(session_id, {}).get("history", [])
        
        # Safely construct formatted history
        formatted_history = "\n".join(
            f"User: {entry.get('query', '[No query found]')}\nAssistant: {entry.get('response', '[No response found]')}"
            for entry in history
        )

        # Return a combined prompt
        prompt = f"""
    ...
    def validate_history(history):
        """Ensure history entries have both 'query' and 'response' keys."""
        for i, entry in enumerate(history):
            if 'query' not in entry or 'response' not in entry:
                logging.warning(f"Invalid history entry at index {i}: {entry}")
        return history
    
    def extract_entities(self, query, response):
        """Extract entities or key information from the query and response."""
        messages = [
            {"role": "system", "content": "You are a knowledge assistant. Extract important entities or key information from the conversation. Return results as key-value pairs."},
    ...
    def analyze_query_context(self, query: str, session_id: str) -> dict:
        """
        Analyze the query in the context of the conversation history.
        Determine if it's a follow-up question and identify needed information.
        """
        # Get the history of the current session
        history = self.memory.get(session_id, {}).get("history", [])
        
        # Debug: Print the session history
        print(f"Debug - Session history for {session_id}: {history}")

        # Format the last few exchanges into a conversation string
        formatted_history = "\n".join(
            f"User: {entry['query']}\nAssistant: {entry['response']}"
            for entry in history[-3:]  # Include only the last 3 exchanges
        )

        # Debug: Print the formatted history
        print(f"Debug - Formatted conversation history:\n{formatted_history}")

        # Create a system prompt for analyzing the query
        messages = [
            {
                "role": "system",
                "content": """
                You are an assistant analyzing user queries. Determine if the query is a follow-up to previous questions.
                Provide context-aware analysis and specify if the query references previous interactions.
                Respond in JSON format:
                {
                    "is_follow_up": boolean,
                    "references": {
                        "info_needed": ["popularity", "genres", etc],
                        "target_type": "artist|track|album|playlist|show|etc",
            ...
            },
            {
                ...
                Analyze the query and provide context as JSON:
                """,
            },
        ]

        try:
            # Send the query to OpenAI for analysis
            response = self.client.chat.completions.create(
                model=model,
                messages=messages,
                max_tokens=200
            ...
            try:
                ...
                return {"is_follow_up": False, "references": {"info_needed": [], "target_type": None}}

        except Exception as e:
            print(f"Error analyzing query context: {e}")
            return {"is_follow_up": False, "references": {"info_needed": [], "target_type": None}}
# class SpotifyClient:
